...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rateLimit = await enforceRateLimit({ key: rateLimitKey(request, "agents:pause", auth.wallet.walletKey), limit: 30, windowMs: 60_000 });
  ...
  const updated = await setAgentStatus(id, "paused");
