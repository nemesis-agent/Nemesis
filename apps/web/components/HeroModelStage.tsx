...
function frameModel(model: Group, T: typeof import("three"), viewportWidth: number) {
  ...
  const targetHeight = viewportWidth < 720 ? 4.2 : 5.7;
...
export function HeroModelStage() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        camera.position.set(0, 0.18, 8.1);
        ...
        scene.add(key);
        const red = new T.PointLight(0xe2524f, 11, 9, 1.8);
        red.position.set(-3.3, 0.8, 2.8);
        scene.add(red);
        const blue = new T.PointLight(0x4a8fd9, 6.4, 8, 2);
        ...
        const halo = new T.Mesh(
          ...
          new T.MeshBasicMaterial({ color: 0xe2524f, transparent: true, opacity: 0.44 }),
        );
        halo.rotation.set(Math.PI / 2, 0, 0);
        halo.position.set(0, -1.12, -0.2);
        scene.add(halo);
        ...
        const tick = () => {
          ...
          model.rotation.y += 0.0024;
          model.rotation.x += ((-pointerY * 0.055 - 0.08) - model.rotation.x) * 0.04;
          model.rotation.z += ((pointerX * 0.035 + 0.02) - model.rotation.z) * 0.04;
          model.position.y = Math.sin(t) * 0.035;
          halo.rotation.z -= 0.004;
          halo.scale.setScalar(1 + drift);
          camera.position.x += (pointerX * 0.16 - camera.position.x) * 0.035;
          camera.position.y += (-pointerY * 0.08 + 0.18 - camera.position.y) * 0.035;
          camera.lookAt(0, 0, 0);
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_42%,rgba(226,82,79,0.18),transparent_30%),radial-gradient(circle_at_58%_54%,rgba(74,143,217,0.12),transparent_26%)]" />
      <div ref={mountRef} className="absolute inset-x-[-22%] bottom-[4%] top-[6%] sm:inset-x-[-8%] sm:bottom-[-2%] sm:top-[2%] lg:inset-x-[4%] lg:bottom-[-4%]" />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,5,5,0.08)_0%,rgba(5,5,5,0.36)_58%,rgba(5,5,5,0.88)_100%)]" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_0%,rgba(5,5,5,0.16)_44%,rgba(5,5,5,0.72)_100%)]" />
      {state === "loading" && (
        <div className="absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 border border-nm-border/70" />
      )}
    </div>
  );
}
